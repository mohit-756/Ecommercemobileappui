  ...
  return (
    <div className="flex flex-col bg-white h-full relative">
      ...
      <div className="absolute top-0 w-full p-5 flex justify-between items-center z-10 pt-12">
        <button onClick={() => navigate(-1)} className="w-10 h-10 bg-white/80 backdrop-blur-md rounded-full flex items-center justify-center text-gray-900 shadow-sm">
          <ArrowLeft size={20} />
        </button>
          ...
          <div className="absolute bottom-4 w-full flex justify-center space-x-2">
            {PRODUCT.images.map((_, i) => (
              <button 
                ...
                className={`w-2 h-2 rounded-full transition-all ${activeImage === i ? "w-6 bg-blue-600" : "bg-gray-300"}`} 
              />
            ))}
          </div>
        </div>
            ...
            <div className="flex flex-wrap gap-3">
              {PRODUCT.sizes.map((size, i) => (
                <button 
                  ...
                  className={`px-4 py-2 rounded-xl border text-sm font-semibold transition-all ${selectedSize === size ? "bg-blue-600 border-blue-600 text-white" : "border-gray-200 text-gray-700 hover:border-gray-300"}`}
                >
                  {size}
                </button>
              ))}
      ...
      {/* Bottom Actions */}
      <div className="absolute bottom-0 w-full bg-white border-t border-gray-100 p-5 pb-safe grid grid-cols-2 gap-4">
